...
import { MSG_OPEN, MSG_PROGRESS, MSG_RESOLVE, SITE, isAliasUrl, isHttpUrl, type LiteshortProgress } from './hosts';
import { resolveDestination } from './resolve';

const LOCATION_RULE = 918700;
...
export const initLiteshortBackground = (): void => {
    ...
    (msg: { type?: string; pageUrl?: string; url?: string }, sender, reply) => {
      ...
      void (async () => {
        try {
          ...
          const push = (p: LiteshortProgress): void => {
            void chrome.runtime.sendMessage({ type: MSG_PROGRESS, ...p }).catch(() => {});
          };
          reply({ ok: true, dest: await resolveDestination(pageUrl, push) });
        } catch {
          reply({ ok: false });
        }
      })();
