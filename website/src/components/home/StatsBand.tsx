...
const stats = [
  { value: String(totalBypasses()), label: 'Countdown & Short-Link Bypasses' },
      ...
      <Shell className="py-12 sm:py-16 lg:py-20">
        <ul className="m-0 grid list-none grid-cols-1 divide-y divide-white/10 p-0 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
          {stats.map((stat) => (
            <li
              key={stat.label}
              className="flex flex-col items-center py-8 text-center first:pt-0 last:pb-0 sm:px-8 sm:py-0"
            >
