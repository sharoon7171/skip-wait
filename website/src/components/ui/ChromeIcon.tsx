...
export function ChromeIcon({ className = '' }: ChromeIconProps): React.ReactElement {
  const uid = useId().replace(/:/g, '');
  const red = `chrome-red-${uid}`;
  const yellow = `chrome-yellow-${uid}`;
  const green = `chrome-green-${uid}`;
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id={red} x1="3.2173" y1="15" x2="44.7812" y2="15" gradientUnits="userSpaceOnUse">
